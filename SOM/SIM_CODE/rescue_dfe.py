...
#Description: Distribution of fitness effects among rescue genotypes
...
def open_output_files(N0, n, U, Es, mmax, mwt, mutmax, nReps):
	...
	data_dir = '../SIM_DATA'
	outfile_A = open("%s/dfe_poisson_%s.csv" %(data_dir,sim_id),"w")
	return outfile_A
...
nReps = 1000 #number of replicates (positive integer)
	...
	while rep < nReps + 1:
		...
		while gen < maxgen + 1:
			...
			if sum(W > 1) > rescue_N:
				...
				rescue_muts = np.sum(genos[rescues], axis=1) - 1 #number of mutations in potential rescues
				ind = np.argmax(counts[rescues]) #choose most common as the rescue genotype
				write_data_to_output(fileHandles, [rescue_growths[ind], rescue_muts[ind]])
				print(rep, "Rescued")              
				break 
