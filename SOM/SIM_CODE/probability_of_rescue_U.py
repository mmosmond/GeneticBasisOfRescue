...
#Description: Probability of rescue for different mutation rates
...
def open_output_files(N0, n, Es, mmax, mwt, mutmax, nReps):
	"""
	This function opens the output files and returns file
	handles to each.
	"""
	sim_id = 'N%d_n%d_Es%.2f_mmax%.2f_mwt%.2f_mutmax%d_nreps%d' %(N0, n, Es, mmax, mwt, mutmax, nReps)
	data_dir = '../SIM_DATA'
...
mwt = -0.1 #malthusian growth rate of wildtype (real) - list to loop over
...
nReps = 10**4 #number of replicates (positive integer)
...
def main():
	...
	# open output file
	fileHandles = open_output_files(N0, n, Es, mmax, mwt, mutmax, nReps) 
	...
	for i in cs:
		...
		print("U =", U, "Fraction rescued =", rescues/nReps)
		write_data_to_output(fileHandles, [i, rescues/nReps])
